...
router.get('/', authenticate, async (req, res) => {
  try {
    const result = await query('SELECT * FROM products WHERE is_active = true ORDER BY name');
...
router.delete('/:id', authenticate, authorize(['admin', 'manager']), async (req, res) => {
  ...
  try {
    // Soft delete: set is_active to false so historical references remain intact
    const result = await query(
    ...
    res.json(successResponse(result.rows[0], 'Product deleted successfully'));
  } catch (error) {
    console.error('Delete product error:', error);
    res.status(400).json(errorResponse(error.message, 'PRODUCT_DELETE_ERROR', 400));
  }
});
