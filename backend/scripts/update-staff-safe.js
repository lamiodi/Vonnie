...
async function clearAndInsertStaff() {
  ...
  try {
    ...
    // Step 1: Handle foreign key dependencies
    console.log('🔄 Handling dependencies...');
    
    // Set created_by to NULL for coupons created by staff
    await client.query(`
      UPDATE coupons 
      SET created_by = NULL 
      WHERE created_by IN (SELECT id FROM users WHERE role IN ('admin', 'manager', 'staff'))
    `);
    
    // Set assigned_by to NULL for booking_workers assigned by staff
    await client.query(`
      UPDATE booking_workers 
      SET assigned_by = NULL 
      WHERE assigned_by IN (SELECT id FROM users WHERE role IN ('admin', 'manager', 'staff'))
    `);
    
    // Delete booking_workers entries for staff
    await client.query(`
      DELETE FROM booking_workers 
      WHERE worker_id IN (SELECT id FROM users WHERE role IN ('admin', 'manager', 'staff'))
    `);
    ...
    // Step 2: Delete existing staff (admin, manager, staff roles)
